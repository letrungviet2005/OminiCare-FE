...
export function AlertDetailDrawer({ alertId, open, onOpenChange, onChanged }: AlertDetailDrawerProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="flex flex-col w-full sm:max-w-md md:max-w-lg lg:max-w-xl">
        {isLoading || !alert ? (
          <DrawerSkeleton />
        ) : (
          <>
            <SheetHeader className="border-b-0 pb-0">
              ...
              <SheetTitle className="mt-3 text-xl leading-tight">{alert.title}</SheetTitle>
              <SheetDescription className="text-sm">{alert.message}</SheetDescription>
              ...
              <section>
                ...
                <div className="mt-2 flex items-start gap-3 rounded-lg border border-slate-200/10 bg-slate-50/60 p-3 dark:border-slate-800 dark:bg-slate-900/60">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-900 dark:text-white">{alert.elderName}</p>
              ...
              <section>
                ...
                <div className="mt-2 flex items-start justify-between gap-3 rounded-lg border border-slate-200/10 bg-slate-50/60 p-3 dark:border-slate-800 dark:bg-slate-900/60">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-[11px] font-mono text-slate-400">ID: {alert.deviceId}</p>
                  </div>
                  <Button variant="outline" size="sm" className="gap-1.5 shrink-0">
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          active ? conf.classes : 'border-slate-200/10 bg-slate-50/40 dark:border-slate-800 dark:bg-slate-900/40 opacity-50'
                        )}
                      >
                        <p className="text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400">Risk</p>
...
function TimelineItem({
  ...
  return (
    <div className="relative flex gap-3 pl-1">
      <div className={cn('flex h-7 w-7 shrink-0 items-center justify-center rounded-full border', toneClass)}>
        <Icon className="h-3.5 w-3.5" />
      </div>
      <div className="min-w-0 flex-1 pb-1">
        <p className="text-sm font-medium text-slate-900 dark:text-white">{title}</p>
        <p className="text-xs text-slate-500 dark:text-slate-400">
          {formatDateTime(time)} · bởi <span className="font-medium">{actor}</span>
        </p>
      </div>
    </div>
  )
